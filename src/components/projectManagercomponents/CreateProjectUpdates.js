...
export default function CreateProjectUpdates(props) {
  ...
  const projectUpdate = async () => {
    ...
    let res = await axios.post(
      " http://localhost:4000/projectManager/createProjectUpdates",
      userObj,
      {
      ...
      >
        ...
        <Modal.Body>
          <form className="w-75">
            ...
            <div className="mt-1">
              <label htmlFor="project_status_update">
                Project Status Update
              </label>
              <textarea
                rows={4}
                {...register("project_status_update", { required: true })}
                className="form-control"
              />
              {errors.project_status_update?.type === "required" && (
                <p className="text-danger">Required*</p>
              )}
            </div>
            <div className="mt-1">
            ...
            <div className="mt-1">
              ...
              <select
                class="form-select"
                aria-label="Default select example"
                {...register("waiting_for_client", { required: true })}
              >
                <option selected disabled>
                  Select
                </option>
              ...
              {errors.waiting_for_client?.type === "required" && (
                <p className="text-danger">Required*</p>
              )}
            </div>
            <div className="mt-2">
