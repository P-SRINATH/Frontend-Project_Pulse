    ...
    <div>
      ...
      <p className='fs-3 text-center'>PROJECT PORTFOLIO DASHBOARD</p>
        <table class="table table-hover table-bordered table-responsive shadow">
            ...
            <tbody>
            {project.map((proObj)=>(<tr key={proObj.project_id}>
            ...
            <td><button className='btn btn-dark'onClick={()=>navigate(`/getspecific/${proObj.project_id}`)}>Details</button></td>
            <td><button className='btn btn-danger' onClick={()=>deleteProj(proObj.project_id)}>
